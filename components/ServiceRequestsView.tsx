      ...
      <div className="flex justify-between items-start mb-4">
        <div>
          <span className={`text-[11px] font-black uppercase tracking-widest ${req.source === 'admin' ? 'text-indigo-600' : 'text-emerald-600'}`}>
            {req.source === 'admin' ? 'Official Task' : req.serviceTitle}
          </span>
         ...
         {req.adminReport && (
           <div className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
             <div className="flex items-center gap-2 mb-2">
               ...
               <p className="text-[10px] font-black text-indigo-600 uppercase">Officer Resolution Report</p>
             </div>
           ...
           >
              <div className="flex items-center gap-3">
                 ...
                 <div className="text-left">
                   <span className="text-[10px] font-black uppercase tracking-widest block opacity-70">Official Doc</span>
    ...
    <div className="h-full flex flex-col animate-slide-in">
      <header className="px-6 pt-12 pb-6 bg-white border-b border-slate-100 flex items-center justify-between shadow-sm">
        <div className="flex items-center gap-5">
          ...
          <h1 className="text-xl font-black text-slate-800 uppercase tracking-tighter">My Gaav Tickets</h1>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 pb-32 hide-scrollbar">
        {showForm ? (
          <div className="bg-white rounded-[32px] p-8 border border-slate-100 shadow-xl space-y-6">
            <h2 className="text-xl font-black text-slate-800 text-center tracking-tight">New Service Request</h2>
            ...
            <textarea rows={4} className="w-full bg-slate-50 border border-slate-100 rounded-2xl p-5 text-sm font-bold outline-none shadow-inner" placeholder="Explain your issue..." value={newReq.description} onChange={(e) => setNewReq({...newReq, description: e.target.value})} />
            <div className="flex gap-3 pt-4">
               <button onClick={() => setShowForm(false)} className="flex-1 py-4 text-xs font-black text-slate-400 uppercase tracking-widest">Cancel</button>
               <button onClick={() => { onNewRequest(newReq.serviceId, newReq.description, userDoc || undefined); setShowForm(false); }} className="flex-2 bg-emerald-600 text-white py-4 rounded-2xl font-black text-sm shadow-lg">Submit Request</button>
            </div>
          </div>
        ) : (
          <div className="space-y-8">
            {adminTasks.length > 0 && (
              <section>
                <div className="flex items-center gap-2 px-2 mb-4">
                  ...
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Official Tasks From Admin</h3>
                </div>
                {adminTasks.map(req => (
                  <RequestItem 
                    key={req.id} 
            ...
            <section>
              <div className="flex items-center gap-2 px-2 mb-4">
                ...
                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">My Service Requests</h3>
              </div>
